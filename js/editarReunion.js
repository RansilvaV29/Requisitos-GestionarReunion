document.addEventListener('DOMContentLoaded', function() {
    ...
    if (isNaN(index)) {
        alert('Índice de reunión inválido.');
        return;
    }
    ...
    document.getElementById('tema').value = reunion.tema;
    document.getElementById('fecha').value = reunion.fecha;
    document.getElementById('hora').value = reunion.hora;
    document.getElementById('objetivo').value = reunion.objetivo;
    document.getElementById('tipo').value = reunion.tipo;
    ...
    let seleccionados = new Set(reunion.participantes);

    function mostrarParticipantes(filtro = '') {
        participantesLista.innerHTML = '';
        participantesLista.appendChild(mensajeNoEncontrado);

        const participantes = ['Juan', 'María', 'Luis', 'Ana', 'Carlos', 'Elena', 'Pedro', 'Sofía', 'Andrés'];
    ...
    document.getElementById('editarReunionForm').addEventListener('submit', function(event) {
        ...
        // Crear objeto de reunión actualizado
        ...
        setTimeout(() => {
            ...
            reunionActualizada.codigoQR = $('#codigoQR canvas')[0].toDataURL();

            // Actualizar la reunión en localStorage
            let reuniones = JSON.parse(localStorage.getItem('reuniones')) || [];
            reuniones[index] = reunionActualizada;
